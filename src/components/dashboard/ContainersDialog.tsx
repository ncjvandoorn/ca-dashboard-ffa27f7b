...
export function ContainersDialog({ open: controlledOpen, onOpenChange, hideTrigger }: ContainersDialogProps = {}) {
  ...
  // servicesOrderId -> ShipperArrival[]
  ...
  // Build a flat row per (container, order) so duplicate order numbers / multi-order containers each get their own row
  type Row = {
    rowKey: string;
